...
async function readCurrentData() {
  try {
    const file = await fs.readFile(CONCERTS_FILE, "utf8");
...
function mapSongkickJsonLdToConcert(item) {
  ...
  const addressCountry = item?.location?.address?.addressCountry;
  if (addressCountry && normalizeText(addressCountry) !== "argentina") return null;
...
function mergeConcerts(currentConcerts, externalConcerts) {
  const currentNormalized = currentConcerts
    .map(normalizeConcert)
    .filter(Boolean);

  const externalNormalized = externalConcerts
    .map(normalizeConcert)
    .filter(Boolean);
...
async function writeData(data) {
  await fs.writeFile(CONCERTS_FILE, `${JSON.stringify(data, null, 2)}\n`, "utf8");
}
...
async function main() {
  ...
  console.log(`Actualización OK: ${nextData.concerts.length} recitales próximos`);
  console.log(`USD/ARS: ${nextData.usdToArs}`);
  console.log(`Nuevos detectados: ${merged.newCount}`);
  if (usdResult.error) console.log(`Aviso USD: ${usdResult.error}`);
  if (ticketmasterResult.error) console.log(`Aviso recitales: ${ticketmasterResult.error}`);
  if (songkickResult.error) console.log(`Aviso fallback recitales: ${songkickResult.error}`);
  if (ticketmasterResult.skipped) console.log("Aviso: TICKETMASTER_API_KEY no configurada, se mantiene lista local.");
}
